...
ENV SIGNALVER 7.9.0
RUN wget -O signal.deb https://github.com/dennisameling/Signal-Desktop/releases/download/v${SIGNALVER}/signal-desktop-unofficial_${SIGNALVER}_arm64.deb

RUN dpkg -i ./signal.deb
RUN rm ./signal.deb
RUN mv "/opt/Signal Unofficial" /opt/signal
...
set -ex \n\
START_COMMAND="/opt/signal/signal-desktop-unofficial" \n\
